...
} // RtmpHosterImpl::SetVideoCapturer
...
void RtmpHosterImpl::StartRtmpStream(const char* url)
{
    if (!av_rtmp_started_) {
        ...
        video_filter_.VBroadcaster().AddOrUpdateSink(
            ((webrtc::AnyRtmpStreamerImpl *) av_rtmp_streamer_)->GetVideoSink(), wants);
...
void RtmpHosterImpl::AddVideoCapturer_w(void* handle)
{
    ...
    if (video_capturer_ != nullptr) {
        ...
        video_capturer_->AddOrUpdateSink(&video_filter_, wants);
        {
            ...
            highest_asked_format.Construct(req_w_, req_h_, cricket::VideoFormat::FpsToInterval(
                                               30), cricket::FourCC::FOURCC_NV12);
...
} // RtmpHosterImpl::AddVideoCapturer_w
