...
public:
    ...
    int32_t SetChannelParameters(uint32_t,
                                 int64_t) override;
...
private:
    ...
    enum libyuv::FourCC encoder_fourcc_;
    int last_set_bitrate_kbps_;
    int last_set_fps_;
    int64_t current_timestamp_us_;
    int frames_received_;
    int frames_encoded_;
    int frames_dropped_media_encoder_;

    int consecutive_full_queue_frame_drops_;
    int64_t stat_start_time_ms_;
    int current_frames_;
    int current_bytes_;
    int current_acc_qp_;
    int current_encoding_time_ms_;
    int64_t last_input_timestamp_ms_;
    int64_t last_output_timestamp_ms_;
    ...
    int32_t output_timestamp_;

    int64_t output_render_time_ms_;

    webrtc::VideoRotation output_rotation_;
    ...
    size_t)
{
    ...
    return codec_thread_->Invoke<int32_t>(
        ...
        Bind(&MediaCodecVideoEncoder::InitEncodeOnCodecThread, this, init_width,
             ...
             codec_settings->maxFramerate,
             false));
} // MediaCodecVideoEncoder::InitEncode
...
int32_t MediaCodecVideoEncoder::SetChannelParameters(uint32_t,
                                                     int64_t)
{
    ...
    int width, int height, int kbps, int fps, bool use_surface)
{
    ...
    inited_ = true;
    return WEBRTC_VIDEO_CODEC_OK;
} // MediaCodecVideoEncoder::InitEncodeOnCodecThread
    ...
    const int64_t                         frame_input_time_ms)
{
    ...
    return WEBRTC_VIDEO_CODEC_OK;
} // MediaCodecVideoEncoder::EncodeOnCodecThread
    ...
    const webrtc::VideoFrame& frame)
{
    ...
    return InitEncodeOnCodecThread(width_, height_, 0, 0, is_texture_frame) ==
           WEBRTC_VIDEO_CODEC_OK;
} // MediaCodecVideoEncoder::MaybeReconfigureEncoderOnCodecThread
                                                      ...
                                                      uint32_t frame_rate)
{
    ...
    return WEBRTC_VIDEO_CODEC_OK;
} // MediaCodecVideoEncoder::SetRatesOnCodecThread
...
bool MediaCodecVideoEncoder::DeliverPendingOutputs(JNIEnv* jni)
{
    ...
    return true;
} // MediaCodecVideoEncoder::DeliverPendingOutputs
    ...
    uint8_t* buffer, size_t buffer_size)
{
    ...
    return -1;
} // MediaCodecVideoEncoder::NextNaluPosition
...
const std::vector<MediaCodecVideoEncoderFactory::VideoCodec>&MediaCodecVideoEncoderFactory::codecs() const
{
