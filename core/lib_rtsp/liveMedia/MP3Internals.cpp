...
MP3FrameParams::MP3FrameParams()
    : bv(frameBytes, 0, sizeof frameBytes)
{
    oldHdr = firstHdr = 0;
...
} // MP3FrameParams::setParamsFromHeader
                                    ...
                                    unsigned& part23Length1bTruncation)
{
    ...
    p23L1 -= p23L1Trunc;
    #ifdef DEBUG
    fprintf(stderr, "updateSideInfoSizes (allowed: %d): %d->%d, %d->%d\n", allowedNumBits, p23L0 + p23L0Trunc, p23L0,
            p23L1 + p23L1Trunc, p23L1);
    #endif
    ...
    sideInfo.ch[1].gr[0].part2_3_length = 0;
    sideInfo.ch[1].gr[1].part2_3_length = 0;
    return p23L0 + p23L1;
} // updateSideInfoSizes
                               ...
                               unsigned& backpointer, unsigned& aduSize)
{
    ...
    fr.setBytePointer(framePtr + 4, totFrameSize - 4);
    ...
    if (totFrameSize < 4 + sideInfoSize) {
        return False;
    }
    ...
    #ifdef DEBUG
    fprintf(stderr,
            "mp3GetADUInfoFromFrame: hdr: %08x, frameSize: %d, part2_3_lengths: %d,%d,%d,%d, aduSize: %d, backpointer: %d\n", hdr, frameSize,
            sideInfo.ch[0].gr[0].part2_3_length, sideInfo.ch[0].gr[1].part2_3_length, sideInfo.ch[1].gr[0].part2_3_length,
                         ...
                         int)
{
    ...
    for (ch = 0; ch < stereo; ch++) {
        ...
        gr_info.part2_3_length         = fr.getBits(12);
        si.ch[ch].gr[1].part2_3_length = 0;
        gr_info.big_values  = fr.getBits(9);
        gr_info.global_gain = fr.getBits(8);
        #if 0
        gr_info.pow2gain = gainpow2 + 256 - gr_info.global_gain + powdiff;
                           ...
                           unsigned newBackpointer)
{
    ...
    fr.setParamsFromHeader();
    fr.setBytePointer(framePtr + 4, totFrameSize - 4);
    if (totFrameSize < 4 + fr.sideInfoSize) {
        return False;
    }
    MP3SideInfo si;
    fr.getSideInfo(si);
    si.main_data_begin = newBackpointer;
                         ...
                         unsigned& availableBytesForBackpointer)
{
    ...
    unsigned desiredOutAduSize =
        (2 * inAduSize * outAveAduSize + inAveAduSize) / (2 * inAveAduSize);
    if (toMaxSize < (4 + outFr.sideInfoSize)) {
        return 0;
    }
