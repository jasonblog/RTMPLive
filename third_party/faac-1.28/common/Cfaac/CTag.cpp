...
int CMP4Tag::WriteMP4Tag(MP4FileHandle MP4File)
{
    ...
    if (artist) {
        MP4SetMetadataArtist(MP4File, artist);
    }

    if (writer) {
        MP4SetMetadataWriter(MP4File, writer);
    }

    if (title) {
        MP4SetMetadataName(MP4File, title);
    }

    if (album) {
        MP4SetMetadataAlbum(MP4File, album);
    }

    if (trackno > 0) {
        MP4SetMetadataTrack(MP4File, trackno, ntracks);
    }

    if (discno > 0) {
        MP4SetMetadataDisk(MP4File, discno, ndiscs);
    }

    if (compilation) {
        MP4SetMetadataCompilation(MP4File, compilation);
    }

    if (year) {
        MP4SetMetadataYear(MP4File, year);
    }

    if (genre) {
        MP4SetMetadataGenre(MP4File, genre);
    }

    if (comment) {
        MP4SetMetadataComment(MP4File, comment);
    }
