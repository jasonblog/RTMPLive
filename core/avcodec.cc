                         ...
                         const size_t nChannels, const uint32_t samplesPerSec, const uint32_t totalDelayMS)
{
    ...
    if (encoder_) {
        ...
        status = aac_encoder_encode_frame(encoder_, (uint8_t *) audioSamples, nSamples * nBytesPerSample * nChannels,
                                          encoded, &outlen);
        if (outlen > 0) {
...
void A_AACEncoder::OnData(const Data& audio)
{
    ...
    if (encoded_) {
        if (audio_record_sample_hz_ != audio.sample_rate || audio.channels != audio_record_channels_) {
            ...
            int samples_per_channel_int = resampler_record_.Resample10Msec((int16_t *) audio.data,
            ...
            Encode((int16_t *) audio.data, audio.samples_per_channel, 2, audio_record_channels_,
                   audio_record_sample_hz_, 0);
        }
    }
}
    ...
    encoder_(NULL)
{
    ...
    h264_.codecSpecific.H264.keyFrameInterval = h264_.maxFramerate * 3;
