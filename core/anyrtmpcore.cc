    ...
    if (audio_record_callback_) {
        if (audio_record_sample_hz_ != samplesPerSec || nChannels != audio_record_channels_) {
            ...
            int samples_per_channel_int = resampler_record_.Resample10Msec((int16_t *) audioSamples,
                                                                           ...
                                                                           audio_record_sample_hz_ * audio_record_channels_, 1, kMaxDataSizeSamples,
                                                                           temp_output);
            audio_record_callback_->OnRecordAudio(temp_output, audio_record_sample_hz_ / 100, nBytesPerSample,
                                                  audio_record_channels_, audio_record_sample_hz_, totalDelayMS);
        } else {
            audio_record_callback_->OnRecordAudio(audioSamples, nSamples, nBytesPerSample, audio_record_channels_,
                                      ...
                                      int64_t* elapsed_time_ms, int64_t* ntp_time_ms)
{
    ...
    if (audio_track_callback_ != NULL) {
        ...
        *ntp_time_ms = 0;
        if (readed > 0) {
            int16_t temp_output[kMaxDataSizeSamples];
            int samples_per_channel_int = resampler_track_.Resample10Msec((int16_t *) aac_buf,
                                                                          audio_track_sample_hz_ * audio_track_channels_,
                                                                          samplesPerSec * nChannels, 1, kMaxDataSizeSamples,
                                                                          temp_output);
