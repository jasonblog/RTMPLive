...
int mp4_set_metadata(MP4FileHandle file, const char* item, const char* val)
{
    ...
    if (!stricmp(item, "track") || !stricmp(item, "tracknumber")) {
        ...
        if (MP4SetMetadataTrack(file, trkn, tot)) {
            return 1;
        }
        ...
        if (MP4SetMetadataDisk(file, disk, tot)) {
            return 1;
        }
        ...
        if (MP4SetMetadataCompilation(file, cpil)) {
            return 1;
        }
        ...
        if (MP4SetMetadataTempo(file, tempo)) {
            return 1;
        }
    } else if (!stricmp(item, "artist")) {
        if (MP4SetMetadataArtist(file, val)) {
            return 1;
        }
    } else if (!stricmp(item, "writer")) {
        if (MP4SetMetadataWriter(file, val)) {
            return 1;
        }
    } else if (!stricmp(item, "title")) {
        if (MP4SetMetadataName(file, val)) {
            return 1;
        }
    } else if (!stricmp(item, "album")) {
        if (MP4SetMetadataAlbum(file, val)) {
            return 1;
        }
    } else if (!stricmp(item, "date") || !stricmp(item, "year")) {
        if (MP4SetMetadataYear(file, val)) {
            return 1;
        }
    } else if (!stricmp(item, "comment")) {
        if (MP4SetMetadataComment(file, val)) {
            return 1;
        }
    } else if (!stricmp(item, "genre")) {
        if (MP4SetMetadataGenre(file, val)) {
            return 1;
        }
    } else if (!stricmp(item, "tool")) {
        if (MP4SetMetadataTool(file, val)) {
            return 1;
        }
    } else {
        if (MP4SetMetadataFreeForm(file, (char*)item, (u_int8_t*)val, (u_int32_t)strlen(val) + 1)) {
            return 1;
        }
    }
