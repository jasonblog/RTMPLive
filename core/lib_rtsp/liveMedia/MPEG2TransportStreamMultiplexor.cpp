...
#define PAT_PERIOD            100
#define PMT_PERIOD            500
                  ...
                  int mpegVersion, MPEG1or2Demux::SCR scr, int16_t PID)
{
    ...
    if (stream_id == 0xBE) {
        ...
        if (fPCR_PID == 0) {
            if ((!fHaveVideoStreams && (streamType == 3 || streamType == 4 || streamType == 0xF)) ||
                (streamType == 1 || streamType == 2 || streamType == 0x10 || streamType == 0x1B || streamType == 0x24))
            {
                fPCR_PID = fCurrentPID;
            }
        }
                      ...
                      unsigned& startPositionInBuffer)
{
    if (fMaxSize < TRANSPORT_PACKET_SIZE) {
        ...
        unsigned numHeaderBytes  = 4;
        unsigned numPCRBytes     = 0;
        ...
        *header++ = pid;
        unsigned& continuity_counter = fPIDState[pid].counter;
            ...
            if (numHeaderBytes > 5) {
                ...
                *header++ = flags;
                if (willAddPCR) {
                    ...
                    *header++ = (u_int8_t) fPCR.extension;
                }
            }
        }
        for (unsigned i = 0; i < numPaddingBytes; ++i) {
            *header++ = 0xFF;
        }
        memmove(header, &buffer[startPositionInBuffer], numDataBytes);
        startPositionInBuffer += numDataBytes;
    }
} // MPEG2TransportStreamMultiplexor::deliverDataToClient
...
{
    unsigned const patSize   = TRANSPORT_PACKET_SIZE - 4;
    ...
    *pat++ = OUR_PROGRAM_NUMBER;
    *pat++ = 0xE0 | (OUR_PROGRAM_MAP_PID >> 8);
    *pat++ = OUR_PROGRAM_MAP_PID;
    u_int32_t crc = calculateCRC(patBuffer + 1, pat - (patBuffer + 1));
    *pat++ = crc >> 24;
    *pat++ = crc >> 16;
    *pat++ = crc >> 8;
    *pat++ = crc;
    while (pat < &patBuffer[patSize]) {
        *pat++ = 0xFF;
    }
...
void MPEG2TransportStreamMultiplexor::deliverPMTPacket(Boolean hasChanged)
{
    ...
    unsigned const pmtSize   = TRANSPORT_PACKET_SIZE - 4;
    unsigned char* pmtBuffer = new unsigned char[pmtSize];
    unsigned char* pmt       = pmtBuffer;
    *pmt++ = 0;
    *pmt++ = 2;
    *pmt++ = 0xB0;
    unsigned char* section_lengthPtr = pmt;
    *pmt++ = 0;
    *pmt++ = OUR_PROGRAM_NUMBER >> 8;
    *pmt++ = OUR_PROGRAM_NUMBER;
    *pmt++ = 0xC1 | ((fProgramMapVersion & 0x1F) << 1);
    *pmt++ = 0;
    *pmt++ = 0;
    *pmt++ = 0xE0;
    *pmt++ = fPCR_PID;
    ...
    *pmt++ = 0;
    ...
    unsigned section_length = pmt - (section_lengthPtr + 1) + 4;
    ...
    *pmt++ = crc >> 8;
    *pmt++ = crc;
    while (pmt < &pmtBuffer[pmtSize]) {
        *pmt++ = 0xFF;
    }
    unsigned startPosition = 0;
    deliverDataToClient(OUR_PROGRAM_MAP_PID, pmtBuffer, pmtSize, startPosition);
    delete[] pmtBuffer;
} // MPEG2TransportStreamMultiplexor::deliverPMTPacket
...
void MPEG2TransportStreamMultiplexor::setProgramStreamMap(unsigned frameSize)
{
    ...
    unsigned offset = 10 + program_stream_info_length;
...
} // MPEG2TransportStreamMultiplexor::setProgramStreamMap
