...
void MPEG1or2Demux
::noteElementaryStreamDeletion(MPEG1or2DemuxedElementaryStream *)
{
...
void MPEGProgramStreamParser::parsePackHeader()
{
    ...
    MPEG1or2Demux::SCR& scr = fUsingDemux->fLastSeenSCR;
    if ((nextByte & 0xF0) == 0x20) {
...
unsigned char MPEGProgramStreamParser::parsePESPacket()
{
    ...
    #endif // if defined(DEBUG) || defined(DEBUG_TIMESTAMPS)
    #ifdef DEBUG
    static unsigned frameCount = 1;
    fprintf(stderr, "%d, saw %s stream: 0x%02x\n", frameCount, streamTypeStr, streamNum);
    fflush(stderr);
    #endif
    ...
    if (fUsingDemux->fMPEGversion == 1) {
        if (!isSpecialStreamId(stream_id)) {
            ...
            if ((nextByte & 0xF0) == 0x20) {
                ...
                #else // ifdef DEBUG_TIMESTAMPS
                skipBytes(9);
                #endif // ifdef DEBUG_TIMESTAMPS
            }
        }
    } else {
        if (!isSpecialStreamId(stream_id)) {
    ...
    if (PES_packet_length < bytesSkipped) {
        ...
        if (out.isCurrentlyAwaitingData) {
            ...
            fprintf(stderr, "%d, %d bytes of PES_packet_data (out.maxSize: %d); first 4 bytes: 0x%08x\n", frameCount,
                    numBytesToCopy, out.maxSize, next4Bytes);
            fflush(stderr);
            #endif
...
} // MPEGProgramStreamParser::parsePESPacket
