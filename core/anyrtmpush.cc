...
void AnyRtmpPush::DoSendData()
{
    ...
    if (dataPtr != NULL) {
        if (dataPtr->_type == VIDEO_DATA) {
            ...
            int ret = srs_rtmp_write_packet(rtmp_, SRS_RTMP_TYPE_SCRIPT, dataPtr->_dts, (char *) dataPtr->_data,
                                            dataPtr->_dataLen);
            if (ret != 0) {
