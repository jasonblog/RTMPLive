...
void MP3StreamState::getAttributes(char* buffer, unsigned bufferSize) const
{
    ...
    unsigned fpt = (unsigned) (filePlayTime() + 0.5);
...
init_resync:
        ...
        if ((fr().hdr & 0xffe00000) != 0xffe00000 ||
            (fr().hdr & 0x00060000) == 0 ||
            (fr().hdr & 0x0000F000) == 0 ||
            (fr().hdr & 0x0000F000) == 0x0000F000 ||
            (fr().hdr & 0x00000C00) == 0x00000C00 ||
            (fr().hdr & 0x00000003) != 0x00000000
        )
        {
            if (fr().hdr == ('R' << 24) + ('I' << 16) + ('F' << 8) + 'F') {
                ...
                #endif
                readFromStream(buf, 66);
                goto read_again;
            }
            if ((fr().hdr & 0xFFFFFF00) == ('I' << 24) + ('D' << 16) + ('3' << 8)) {
                unsigned tagSize, bytesToSkip;
                unsigned char buf[1000];
                readFromStream(buf, 6);
                tagSize = ((buf[2] & 0x7F) << 21) + ((buf[3] & 0x7F) << 14) + ((buf[4] & 0x7F) << 7)
                          + (buf[5] & 0x7F);
                bytesToSkip = tagSize;
                while (bytesToSkip > 0) {
...
{
    if (fr().frameSize < fr().sideInfoSize) {
        return;
    }
    ...
    if (flags & XING_TOC_FLAG) {
        if (bytesAvailable < XING_TOC_LENGTH) {
            return;
        }
        fHasXingTOC = True;
...
} // MP3StreamState::checkForXingHeader
