...
int main(int argc, char* argv[])
{
    ...
    while (argc - optind > 0) {
        ...
        if (container == MP4_CONTAINER) {
            ...
            if (artist) {
                MP4SetMetadataArtist(MP4hFile, artist);
            }

            if (writer) {
                MP4SetMetadataWriter(MP4hFile, writer);
            }

            if (title) {
                MP4SetMetadataName(MP4hFile, title);
            }

            if (album) {
                MP4SetMetadataAlbum(MP4hFile, album);
            }

            if (trackno > 0) {
                MP4SetMetadataTrack(MP4hFile, trackno, ntracks);
            }

            if (discno > 0) {
                MP4SetMetadataDisk(MP4hFile, discno, ndiscs);
            }

            if (compilation) {
                MP4SetMetadataCompilation(MP4hFile, compilation);
            }

            if (year) {
                MP4SetMetadataYear(MP4hFile, year);
            }

            if (genre) {
                MP4SetMetadataGenre(MP4hFile, genre);
            }

            if (comment) {
                MP4SetMetadataComment(MP4hFile, comment);
            }
